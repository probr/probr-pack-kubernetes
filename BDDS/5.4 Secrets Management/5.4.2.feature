...
@preventitive
Scenario Outline: Prevent secrets from being accessed inside the kuburnetes cluster
Given an active kuburnetes cluster exists 
And some system exists to detect when someone attempts to access secrets
When someone tries to access secrets inside the kuburnetes cluster
And the access limited is <Access> 
Then the attempt will <Result> with error <Error Message>

Examples:
    | Access    | Result      | Error Message                                            |
    | True      | Fail        | Secrets can't be accessed inside the kuburnetes cluster |
